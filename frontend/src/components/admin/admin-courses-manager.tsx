      ...
      <div className="rounded-[2rem] border border-slate-200 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 p-8 text-white shadow-sm">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-base font-medium text-slate-300">{subheading}</p>
          </div>
          <Button
            onClick={() => {
            ...
            variant="secondary"
            className="h-[54px] px-6 text-base font-bold"
          >
          ...
          <Card key={course.id} className="rounded-3xl border-slate-200 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="space-x-2">
                  <Button variant="outline" size="sm" onClick={() => { setEditingCourse(course); setIsModalOpen(true); }}>
                    Edit Course
                  </Button>
            ...
            <CardContent>
              ...
              <div className="space-y-6">
                {course.units?.length === 0 && <span className="italic text-muted-foreground">No units yet.</span>}
                  ...
                  <div key={unit.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      <div className="flex space-x-2">
                        <Button variant="ghost" size="sm" onClick={() => { setEditingUnit(unit); setIsUnitModalOpen(true); }}>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {unit.lessons?.map((lesson) => (
                        <div key={lesson.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-3">
                          <div className="mb-2 flex items-start justify-between">
                            <span className="text-sm font-semibold text-neutral-800">
                              Lesson {lesson.order_index}: {lesson.title}
                            </span>
                            <div className="flex space-x-1">
                              <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => { setEditingLesson(lesson); setIsLessonModalOpen(true); }}>
